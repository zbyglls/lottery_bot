...
async def create_lottery(
    ...
    try:
        ...
        lottery_settings = {
            ...
            'description': description,
            'join_method': join_method,
            'keyword_group_id': keyword_group_id if keyword_group_id else '',
            'keyword': keyword if keyword else '',
            'message_group_id': message_group_id if message_group_id else '',
            'message_count': message_count if message_count else '',
            'message_check_time': message_check_time if message_check_time else '',
            'require_username': require_username.lower() in ('true', '1', 'yes') if require_username else False,
            'required_groups': group_ids if group_ids else '',
            'draw_method': draw_method,
            'participant_count': participant_count if participant_count else '',
